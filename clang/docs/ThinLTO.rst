...
DTLTO is currently only implemented by the LLD linker (``-fuse-ld=lld``).
...
compiler to be executed remotely to perform the ThinLTO backend compilations.
Currently, this is Clang itself.

Usage Examples:

Compilation is unchanged from ThinLTO, see above.

.. code-block:: console

  % clang -flto=thin -fthinlto-distributor=incredibuild -Xthinlto-distributor=--verbose,--j10 -fuse-ld=lld file1.o file2.o
  % clang -flto=thin -fthinlto-distributor=$(which python) -Xthinlto-distributor=incredibuild.py -fuse-ld=lld file1.o file2.o

When using lld-link:

.. code-block:: console

  % lld-link /out:a.exe file1.obj file2.obj /thinlto-distributor:incredibuild.exe /thinlto-remote-compiler:${LLVM}\bin\clang.exe
